...
//#define USE_SYSTEM_MALLOC
    ...
    #define slab_alloc(_size) ({size_t __size=(_size); void *_ret_ptr_=_slab_alloc(__size); printf("%s [%s:%d]%p=slab_alloc(%u)\n", __func__, __FILE__, __LINE__, _ret_ptr_, (unsigned int)(__size)); _ret_ptr_;})
#else
...
#ifdef TRACE_MEM_ALLOC
    #define slab_realloc(_ptr, _size) ({void* __ptr=_ptr; size_t __size=(_size); void *_ret_ptr_=_slab_realloc(__ptr, __size); printf("%s [%s:%d]%p=slab_realloc(%p, %u)\n", __func__, __FILE__, __LINE__, _ret_ptr_, __ptr, (unsigned int)(__size)); _ret_ptr_;})
#else
...
#ifdef TRACE_MEM_ALLOC
    #define slab_free(_ptr) ({void* __ptr=(_ptr); printf("%s [%s:%d]slab_free(%p)\n", __func__, __FILE__, __LINE__, __ptr); _slab_free(__ptr);})
#else
...
#ifdef TRACE_MEM_ALLOC
    #define slab_calloc(_size) ({size_t __size=(_size); void *_ptr=_slab_calloc(__size); printf("%s [%s:%d]%p=slab_calloc(%u)\n", __func__, __FILE__, __LINE__, _ptr, (unsigned int)(__size)); _ptr;})
#else
