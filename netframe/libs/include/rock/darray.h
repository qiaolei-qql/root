 ...
 * +-----------------------------------------------------------------------+
 *
 * $Id: gdarray.h,v 1.5 2008/10/27 07:42:34 jingmi Exp $
 */
/**
 * @file        darray.h
 * @author      jingmi@gmail.com
 ...
 * | 1. PREDEFINED MACRO                              |
 * +--------------------------------------------------+
 * | MUL_THREAD, DARRAY_NODE_SIZE, DARRAY_DEBUG_MODE  |
 * | mem_alloc, mem_free                              |
 * +--------------------------------------------------+
 * | 2. TYPEDEF                                       |
 * +--------------------------------------------------+
 * | darray                                           |
 * +--------------------------------------------------+
 * | 3. FUNCTION/MACRO LIST                           |
 * +--------------------------------------------------+
 * | darray_new, darray_new2                          |
 * | darray_new_ptr, darray_declare                   |
 * | darray_free                                      |
 * | darray_exlock, darray_shlock, darray_unlock      |
 * | darray_foreach, darray_all, darray_map           |
 * | darray_size, darray_setsize                      |
 * | darray_addr, darray_push                         |
 * | darray_shrink_size, darray_clear                 |
 * +--------------------------------------------------+
 */
 ...
 * |2010-04-20 15:27                                  |
 * +--------------------------------------------------+
 * |1. function/macro list                            |
 * |2. darray_clear()                                 |
 * |3. darray_map() : process setsize()               |
 * |4. darray_all() : process node_size*node_count    |
 * |5. darray_foreach() : provide counter             |
 * +--------------------------------------------------+
 */
...
#define darray_declare(head_ptr) \
    struct darray_common_head *head_ptr
...
#define darray_foreach(head_ptr, item_struct, item_ptr, _max_, CODES) \
    for (size_t _my_counter_=0; _my_counter_<(size_t)(_max_); _my_counter_++)  \
    {                           \
        struct item_struct *item_ptr = darray_addr(head_ptr, _my_counter_); \
        do{ CODES } while(0);   \
    }

#define darray_all(head_ptr, item_struct, item_ptr, CODES)  \
    darray_foreach(head_ptr, item_struct, item_ptr, (((head_ptr)->node_size)*((head_ptr)->node_count)), CODES)

#define darray_map(head_ptr, item_struct, item_ptr, CODES)  \
    darray_foreach(head_ptr, item_struct, item_ptr, (darray_setsize(head_ptr)), CODES)
...
{
    // TODO: change index to int64_t is much better
    void *item_ptr;

    _darray_addr(head_ptr, index, item_ptr);
    if (index > 0)
    {
        if (index > (int)darray_setsize(head_ptr))
            darray_setsize(head_ptr) = index+1;
    }
...
/* {{{ static inline int darray_clear(struct darray_common_head *head_ptr) */
static inline int
darray_clear(struct darray_common_head *head_ptr)
{
    struct darray_common_node *node_ptr;

    node_ptr = head_ptr->last_node_ptr;
    while (node_ptr != head_ptr->first_node_ptr)
    {
        struct darray_common_node *tmp = node_ptr->pre_node;
        mem_free(node_ptr);
        node_ptr = tmp;
    }
    head_ptr->node_count = 1;
    head_ptr->capacity = head_ptr->node_size;
    head_ptr->setsize = 0;
    head_ptr->last_node_ptr = head_ptr->first_node_ptr;
    head_ptr->cache_node_subscript = 0;
    head_ptr->cache_node_ptr = head_ptr->first_node_ptr;
    head_ptr->cache_item_subscript = 0;
    head_ptr->cache_item_ptr = &(head_ptr->first_node_ptr->item_array[0]);
    head_ptr->first_node_ptr->id = 0;
    head_ptr->first_node_ptr->offset = 0;
