...
bool SDRedisServer::connect(uint32_t toms)
{
	...
	{
		LOG_DEBUG("redis server:" << dump() << " has been connected, close it at first and reconnect");
		close();
	}
	...
	{
		LOG_ERROR("connect to " << dump() << " failed.");
		close();
		return false;
	}

	LOG_DEBUG("connect to " << dump() << " ok.");
...
bool SDRedisServer::ping()
{
	if(!is_connected())
	{
		LOG_WARN("ping " << dump() << " but is not connected!");
		return false;
	}
	...
	if(NULL == reply)
	{
		LOG_WARN("ping " << dump() << " reply null, close connection");
		close();
		return false;
	}

	LOG_DEBUG("ping " << dump() << " reply:" << std::string(reply->str));
...
bool SDRedisServer::slaveofnoone()
{
	if (!is_connected()){
		LOG_WARN("slave of no one " << dump() << " but is not connected!");
		return false;
	}
	bool succ = false;
	redisReply *reply = (redisReply *)redisCommand(m_redis_context, "SLAVEOF NO ONE");
	if (NULL == reply)
	{
		LOG_WARN("slave of no one " << dump() << " reply null, close connection");
		close();
		return false;
	}
	LOG_DEBUG("slave of no one " << dump() << " reply:" << std::string(reply->str));
...
bool SDRedisServer::slaveof(const std::string host, uint16_t port)
{
	if (!is_connected()){
		LOG_WARN("slave of no one " << dump() << " but is not connected!");
		return false;
	...
	bool succ = false;
	redisReply *reply = (redisReply *)redisCommand(m_redis_context, "SLAVEOF %s %d",(char*)host.c_str(),port);
	if (NULL == reply)
	{
		LOG_WARN("slave of no one " << dump() << " reply null, close connection");
		close();
	...
	LOG_DEBUG("slave of no one " << dump() << " reply:" << std::string(reply->str));
	cout <<"slave of no one " << dump() << " reply:" << std::string(reply->str)<<endl;
	if(reply->type == REDIS_REPLY_STATUS  && std::string(reply->str) == "OK")
	{
		succ = true;
	}
...
bool SDRedisServer::sync_status()
{
	...
	if (1 !=info_map.count("master_link_status")){
		LOG_DEBUG("have no master_link_status info redis is not slave");
		return false;
	}
	if (info_map["master_link_status"] != "up")
	{
		LOG_DEBUG("master_link_status:"<< info_map["master_link_status"]<< "len:"<<info_map["master_link_status"]<<";");
		return false;
	}
	return true;
}
bool SDRedisServer::info(std::map<std::string,std::string>& info_map)
{
	if (!is_connected()){
		LOG_WARN("info  " << dump() << " but is not connected!");
		return false;
	}
	bool succ = false;
	redisReply *reply = (redisReply *)redisCommand(m_redis_context, "INFO");
	if (NULL == reply)
	{
		LOG_WARN("info  " << dump() << " reply null, close connection");
		close();
		return false;
	}
	LOG_DEBUG("info  " << dump() << " reply:" << std::string(reply->str));
	if(reply->type != REDIS_REPLY_STRING)
	{
		LOG_ERROR(" reply type:" << reply->type );
		freeReplyObject(reply);
		return false;
	}
	std::string str_info(reply->str);
	std::vector<std::string> str_line_vec;
	std::string filed,value;
	boost::split(str_line_vec,str_info,boost::is_any_of("\n"));
	for (int i=0; i<str_line_vec.size();i++){
		std::vector<std::string> str_key_val;
		boost::split(str_key_val,str_line_vec[i],boost::is_any_of(":"));
		if (2 !=str_key_val.size()){
			continue;
		}
		info_map[str_key_val[0]] = str_key_val[1].substr(0,str_key_val[1].length()-1);
	}

	LOG_DEBUG( "redisCommand info " <<  " succ ,reply info map size:" << info_map.size());
	freeReplyObject(reply);
	return true;
}
bool SDRedisServer::select_db(uint16_t db_index)
{
	if(!is_connected())
	{
		LOG_WARN("select " << dump() << " but is not connected!");
		return false;
	}
	...
	if(!reply)
	{
		...
		LOG_ERROR( "redisCommand(" << m_redis_context << ","  << command.str() << ") failed! err:" << m_redis_context->err << "(" << string(m_redis_context->errstr) << ")");
		close();
		return -1;
	}
...
int SDRedisServer::hrem(const std::string &key, std::vector<Value> &values)
{
	...
	{
		//Once an error is returned the context cannot be reused and you should set up a new connection
		LOG_ERROR( "redisCommand(" << m_redis_context << ","  << command << ") failed! err:" << m_redis_context->err << "(" << string(m_redis_context->errstr) << ")");
		close();
		return -1;
	}
	LOG_DEBUG( "redisCommand " << command << " succ, reply type:" << reply->type);
	...
	{
		LOG_DEBUG( "rem key:" << key << " 's filed in hash return " << reply->integer << "/" << size);
	}
...
int SDRedisServer::hget(const std::string &key, std::vector<Value> &values)
{
	...
	{
		LOG_ERROR("HMGET key:" << key << " reply type:" << reply->type << "fileds:" << size << "/" << reply->elements);
		freeReplyObject(reply);
		return 0;
	}
	...
	LOG_DEBUG( "redisCommand " << command << " succ ,reply fileds size:" << reply->elements << ", has_date:" << has_data);
	freeReplyObject(reply);
...
int SDRedisServer::hset(const std::string &key, std::vector<Value> &values)
{
	...
	{
		LOG_ERROR("HMSET key:" << key << " reply type:" << reply->type);
		freeReplyObject(reply);
		return 0;
	}
...
int SDRedisServer::strset2(const std::string &key,const std::string &value, uint32_t duration_time)
{
	...
	LOG_DEBUG( "redisCommand " << command.str() << " succ, reply type:" << reply->type);
	...
	LOG_DEBUG("SETEX key:" << key << " reply type:" << reply->type << " value len:" << value.length());
	freeReplyObject(reply);
	return ret;
...
int SDRedisServer::exist(const std::string &key, bool &exist)
{
	...
	if(reply->type == REDIS_REPLY_INTEGER)
	{
		if(reply->integer == 1)
		{
			LOG_DEBUG( "key:" << key << " exist in cache");
			exist = true;
		}
		else
		{
			exist = false;
			LOG_WARN( "key:" << key << " not exist in cache");
		}
	}
...
int SDRedisServer::remove(const std::string &key)
{
	...
	if(reply->type == REDIS_REPLY_INTEGER)
	{
		LOG_DEBUG( "remove key:" << key << " in cache return " << reply->integer);
	}
...
int SDRedisServer::expire(const std::string &key, uint32_t duration_time)
{
	...
	if(reply->type == REDIS_REPLY_INTEGER)
	{
		LOG_DEBUG( "expire key:" << key << " in cache return " << reply->integer);
		if(reply->integer == 1)
		{
			ret = 1;
		}
...
int SDRedisServer::hincrby(const std::string &key,const std::string &filed,int32_t &increment)
{
	...
	if(reply->type == REDIS_REPLY_INTEGER)
	{
		LOG_DEBUG( "HINCRBY key:" << key << " in cache return " << reply->integer);
		ret =1;
	}
	else
	{
		LOG_ERROR( "HINCRBY key:" << key << " in cache return " << reply->str);
		ret = 0;
	}
...
int SDRedisServer::migrate(std::string &dst_redis_ip, uint16_t dst_redis_port, const std::string &key, uint32_t db, uint32_t to_ms)
{
	...
	if(reply->type == REDIS_REPLY_STATUS)
	{
		if(std::string(reply->str,reply->len) == "OK")
		{
			LOG_DEBUG( "MIGRATE key:" << key << " return " << std::string(reply->str,reply->len));
			ret = 1;
		}
		else
		{
			LOG_WARN( "MIGRATE key:" << key << " return " << std::string(reply->str,reply->len));
			ret = 0;
		}
	}
...
int SDRedisServer::sadd(const std::string &key, std::vector<std::string> &members)
{
	...
	if(reply->type == REDIS_REPLY_INTEGER)
	{
		LOG_DEBUG( "add key:" << key << " 's member in set return " << reply->integer << "/" << size);
	}
...
int SDRedisServer::sget(const std::string &key, std::vector<std::string> &members)
{
	...
	if(reply->type != REDIS_REPLY_ARRAY)
	{	
		LOG_ERROR("SMEMBERS key:" << key << " reply invalid type:" << reply->type);
		freeReplyObject(reply);
		return 0;
	}
	...
	LOG_DEBUG("SMEMBERS key:" << key << " reply type:" << reply->type << " member size:" << reply->elements);
	freeReplyObject(reply);
	return 1;
...
int SDRedisServer::srem(const std::string &key, std::vector<std::string> &members)
{
	...
	if(reply->type == REDIS_REPLY_INTEGER)
	{
		LOG_DEBUG( "rem key:" << key << " 's member in set return " << reply->integer << "/" << size);
	}
...
int SDRedisServer::strget(const std::string &key, std::string &value)
{
	...
	LOG_DEBUG("GET key:" << key << " reply type:" << reply->type << " value len:" << value.length());
	freeReplyObject(reply);
	return ret;
