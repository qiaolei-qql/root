...
function! s:ProjectUpdate()
    ...
    " create tags file
    if executable('ctags')
        call system("find " . getcwd() .  " -type f -name '*.h' -o -name '*.c' -o -name '*.cpp'   >  "  . proj_data . "/tag.files ")
        echo "find " .getcwd() . " -type f -name '*.h' -o -name '*.c' -o -name '*.cpp'  >  "  . proj_data . "/tag.files "

        call system('ctags -R --c++-kinds=+p --fields=+iaS --extra=+q -o ' . proj_data . '/tags '. ' -L '  . proj_data . '/tag.files ')
        echo 'ctags -R --c++-kinds=+p --fields=+iaS --extra=+q -o ' . proj_data . '/tags '. ' -L '  . proj_data . '/tag.files '
        "call system('ctags -R --c++-kinds=+p --fields=+iaS --extra=+q -o ' . proj_data . '/tags '. ' -L '  . proj_data . '/tag.files ' . getcwd())
    else
        call s:WarnMsg("command 'ctags' not exist.")
        return -1
    endif
...
function! s:ProjectLoad()
    ...
    " load tags.
    let &tags = 'tags'
    let &tags = getcwd() . '/' . proj_data . '/tags,' . &tags
    "let &tags = getcwd() . '/' . proj_data . '/tags'
    ...
    if filereadable(proj_data . '/cstags')
        ...
        set cst
        set nocsverb
        exe 'cs add '.  getcwd() . '/' . proj_data . '/cstags'
        cs reset
        set csverb
    endif
